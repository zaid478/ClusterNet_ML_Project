...
if run_decision:
    for t in range(args.train_iters):
        ...
        #link prediction setting: get loss with respect to training edges only
            ...
            # print(model_cluster(features_train, adj_train, num_cluster_iter))
    ...
    if args.objective == 'kcenter':
        ...
        print('ClusterNet {} value'.format(args.graph_embedding), testvals[np.argmin(trainvals)])
    if args.objective == 'modularity':
            print('ClusterNet {} value'.format(args.graph_embedding), curr_test_loss)
